   ...
   #Serie de fonctions gérant la deconnexion
    ...
    socket.on 'change-title', (message) ->
      ...
      if message.password == admin_password 
        ...
        livedraw_iframe = message.iframe_code
        io.sockets.emit 'new-drawings',livedraw_iframe
        maj_S3episode()
...
if process.env.AWS_ACCESS_KEY_ID == undefined || process.env.AWS_SECRET_ACCESS_KEY == undefined
  ...
  credentials = {}
  c_file = __dirname + '/credentials.json'
  fs.readFile c_file, 'utf8', (err, data) ->
    if (err)
    ...
    AWS.config.accessKeyId = credentials.AWS_KEY 
    AWS.config.secretAccessKey = credentials.AWS_SECRET

    console.log "Use of #{AWS.config.accessKeyId} and #{AWS.config.secretAccessKey}" 
